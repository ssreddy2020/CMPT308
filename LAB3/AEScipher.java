...
public class AEScipher {
	 ...
	 * aesMixColumn
	 * 
	 * This method mix column multiplication by using mc2 and mc3 look-up tables
	 * 
	 * @param inStateHex
	 *            is the input 4*4 matrix
	 * 
	 * @return outStateHex is the output 4*4 matrix
	 */
	...
	private static String[][] aesMixColumn(String[][] inStateHex) {
		...
		int in[] = new int[4];
		int out[] = new int[4];
		HashMap<Integer, Integer> multiplyBy2 = new HashMap<Integer, Integer>();
		HashMap<Integer, Integer> multiplyBy3 = new HashMap<Integer, Integer>();
		for (int i = 0; i <= 255; i++) {
			multiplyBy2.put(i, mc2[i]);
		}
		for (int j = 0; j <= 255; j++) {
			multiplyBy3.put(j, mc3[j]);
		}
		for (int x = 0; x <= 3; x++) {
			for (int y = 0; y <= 3; y++) {
				in[y] = Integer.parseInt(inStateHex[y][x], 16);
			}
			out[0] = multiplyBy2.get(in[0]) ^ multiplyBy3.get(in[1]) ^ in[2]
					^ in[3];
			out[1] = in[0] ^ multiplyBy2.get(in[1]) ^ multiplyBy3.get(in[2])
					^ in[3];
			out[2] = in[0] ^ in[1] ^ multiplyBy2.get(in[2])
					^ multiplyBy3.get(in[3]);
			out[3] = multiplyBy3.get(in[0]) ^ in[1] ^ in[2]
					^ multiplyBy2.get(in[3]);

			for (int y = 0; y <= 3; y++) {
				outStateHex[y][x] = Integer.toHexString(out[y]);
			}
		}
		return outStateHex;
	}
